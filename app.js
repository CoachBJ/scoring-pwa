...
// Use this to separate *different* plays while keeping "TD + PAT" together in one pill
const JOINER = " • ";
...
// Strict mm:ss → seconds (00:00 to 24:00). Returns null if invalid.
const fromMMSS = (txt) => {
...
function scoreCombos(target) {
  ...
  function dfs(rem, startIdx) {
    ...
    for (let i = startIdx; i < SCORING_PLAYS.length; i++) {
...
function rankKey(counts) {
  ...
  // prefer fewer plays; then prefer more 8s/7s/6s; fewer FGs/Safeties
...
function formatCombo(counts) {
  ...
  for (let i = 0; i < counts.length; i++) {
    ...
    if (!c) continue;
    const label =
      c > 1 ? `${c}x ${SCORING_PLAYS[i].label}` : SCORING_PLAYS[i].label;
    parts.push(label); // keep "TD + PAT" intact
  }
  return parts.join(JOINER); // separate different plays with a dot
}
...
function renderBanner(our, opp) {
  ...
  if (our < opp) {
    cls = "bad";
    title = `We trail by ${usBehind}`;
  }
  if (our > opp) {
    cls = "good";
    title = `Opponent trails by ${oppBehind}`;
  }
...
// ----- State (persist across app restarts) -----
const STATE_KEY = "scoring-state-v2";
...
function saveState() {
  const state = {
    ...
    ourToH1: Number(elOurToH1.value),
    oppToH1: Number(elOppToH1.value),
    ourToH2: Number(elOurToH2.value),
    oppToH2: Number(elOppToH2.value),
    ball: elBallThem.checked ? "them" : "us",
    snaps: Number(elSnaps.value),
    pclk: Number(elPlayClock.value),
    ptime: Number(elPlayTime.value),
  };
...
function loadState() {
  try {
    ...
    if (s.ball === "them") elBallThem.checked = true;
    else elBallUs.checked = true;
...
function updateTimeLabel() {
  const secs = Number(elTime.value);
  const mmss = toMMSS(secs);
  elTimeLbl.textContent = mmss;
  if (document.activeElement !== elTimeInput) elTimeInput.value = mmss; // don't fight user typing
}

// ----- TO adjust + time tweak buttons -----
...
// Timeout inputs clamp + persist
[elOurToH1, elOppToH1, elOurToH2, elOppToH2].forEach((el) => {
  el.addEventListener("change", () => {
    el.value = clamp(Number(el.value), 0, 3);
    saveState();
    updateClockHelper();
  });
});
...
//   With ourTO timeouts, they can still drain ≈ (N * avgPlayTime) + max(0, N - ourTO) * playClock
function updateClockHelper() {
  ...
  if (elBallUs.checked) {
    ...
    elClockResult.innerHTML =
      `We have ball. Opp TOs: <b>${oppTO}</b>. ` +
      `With <b>${snaps}</b> snaps, est burn ≈ <b>${toMMSS(canBurn)}</b> (capped by time left). ` +
      (remain === 0
        ? `<b>You can run out the half.</b>`
        : `About <b>${toMMSS(remain)}</b> would still remain.`);
  } else {
    const drain = snaps * ptime + Math.max(0, snaps - ourTO) * pclk;
    const canDrain = Math.min(timeLeft, drain);
    elClockResult.innerHTML =
      `They have ball. Our TOs: <b>${ourTO}</b>. ` +
...
function run() {
  ...
  if (our == null || opp == null) {
    elStatus.textContent = "Enter both scores.";
    return;
  }
  ...
  elStatus.textContent = `Score: Us ${our} — Them ${opp} • Us behind: ${usBehind} • Opp behind: ${oppBehind}. Showing exact-point combos.`;
